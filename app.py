...
def write_and_run_temp_python(code: str, injected_pickle: str = None, timeout: int = 60) -> Dict[str, Any]:
    ...
    if injected_pickle:
        ...
        # fallback empty DataFrame so df is always defined
        preamble.append("df = pd.DataFrame()\n")
        preamble.append("data = []\n")

     # Add safe column accessor
        # Add safe column accessor
    safe_get_helper = textwrap.dedent("""\
    def safe_get(df, col):
    ...
    script_lines.append(safe_get_helper)
...
6. Always check that required columns exist (e.g., 'sales', 'region', 'date') before using them.
   If missing, return a friendly message like "Column 'sales' not found in dataset".
"""),
    ("human", "{input}"),
